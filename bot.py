...
tickers = ['BTCUSDT', 'ETHUSDT', 'XRPUSDT', 'DOGEUSDT', 'LTCUSDT']
...
Позиция: LONG
Текущая цена: {df_lower_tf.iloc[-i]['close']}
Значение RSX: {df_lower_tf.iloc[-i]['RSX']}
Цена стопа: {df_lower_tf.iloc[-(i+1)]['low']}
Время сигнала: {df_lower_tf.iloc[-i]['date']}"""
...
Текущая цена: {df_lower_tf.iloc[-i]['close']}
Значение RSX: {df_lower_tf.iloc[-i]['RSX']}
Цена стопа: {df_lower_tf.iloc[-(i+1)]['high']}
Время сигнала: {df_lower_tf.iloc[-i]['date']}"""
                        loop = asyncio.new_event_loop()
                        asyncio.set_event_loop(loop)
                        loop.run_until_complete(message(stringTOsend))
